...
const generateGameHash = (playerAddress, timestamp, pairsMatched) => {
  return ethers.utils.keccak256(
    ethers.utils.defaultAbiCoder.encode(
      ["address", "uint256", "uint256"],
      [playerAddress, timestamp, pairsMatched]
    )
  );
};

const submitToBlockchain = async (level, timeTaken, wrongMoves, pairsMatched) => {
  if (!contractInstance || !selectedAccount) {
    console.warn("Blockchain not connected");
    return;
  }

  try {
    console.log("\nethers :",ethers); // Check if ethers is defined

    // const gameHash = generateGameHash(selectedAccount, Date.now(), pairsMatched);
    const tx = await contractInstance.recordResult(
      level,
      timeTaken,
      wrongMoves,
      pairsMatched
    ...
    // Get updated balance
    const newBalance = await contractInstance.playerCoins(selectedAccount);
    console.log( "\ntx:", tx, "\nnewBalance", newBalance)
    const won = pairsMatched === (level + 2);
    
    return {
      success: true,
      coinsChange: won ? 10 : -(wrongMoves * 2),
      newBalance
    };
  } catch (error) {
  ...
  useEffect(() => {
    if (matchedCards.length === cards.length && cards.length > 0) {
      const handleGameCompletion = async () => {
        ...
        console.log("\ntimer:", timer, "\nfailedattempts", failedAttempts, "\nmatchcards:", matchedCards.length/2)
        ...
        if (blockchainResult.success) {
          const { coinsChange, newBalance } = blockchainResult;
          alert(
            `Game completed!\n` +
            `Record saved to blockchain.\n` +
            `${coinsChange > 0 ? '+' : ''}${coinsChange} coins\n` +
            `New balance: ${newBalance} coins`
          );
        } else {
          alert("Game saved locally (blockchain submission failed)");
        }
